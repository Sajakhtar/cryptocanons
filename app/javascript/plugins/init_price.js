const fetchPrice = (priceElements) => {
  priceElements.forEach((priceEl) => {
    fetch(
      `https://api.coingecko.com/api/v3/coins/${priceEl.dataset.coingeckoId}`
    )
      ...
      .then((data) => {
        ...
        const iconDownArrow = `<i class="fas fa-arrow-down"></i>`;
        const iconUpArrow = `<i class="fas fa-arrow-up"></i>`;
        const icon = change24hr > 0 ? iconUpArrow : iconDownArrow;
        const change24hrEl = priceEl.nextElementSibling;
        change24hrEl.innerHTML = `${icon} ${change24hrFormatted}% 24h`;
